...
async function testrpa() {
    ...
    AlarmContract.on('Plus', async (plus) => {
      if (plus != 25) {
        const updatedtime = date.addMonths(now, 1);
        ...
        const subscribe2 = onSnapshot(colRefNoti2, (querySnapshot) => {
          const address2 = [];
          querySnapshot.forEach((doc) => {
        ...
        const subscribe3 = onSnapshot(colRefNoti3, (querySnapshot) => {
          const address3 = [];
          querySnapshot.forEach((doc) => {
        ...
        const subscribe4 = onSnapshot(colRefOverdue, (querySnapshot) => {
          const address4 = [];
          querySnapshot.forEach((doc) => {
